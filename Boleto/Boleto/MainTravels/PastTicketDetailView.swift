                    ...
                    HStack {
                        ...
                        NumsParticipantsView(personNum: store.ticket.participant.count)
                            .onTapGesture {
                                ...
                                store.send(.tapNums(CGPoint(x: frame.maxX - 100, y: frame.maxY)))
                            }
            ...
            if store.showingModal {
                ZStack {
                    Color.black.opacity(0.5)
                        ...
                        .onTapGesture {
                            store.send(.hideticket)
                        }
                    TravelCompanionsView(persons: store.ticket.participant)
                        .position(x: store.modalPosition.x , y: store.modalPosition.y + 20)
                }
            }
        }
