...
struct TravelResponse: Decodable {
    ...
    func toTicket() -> Ticket {
        ...
        return .init( travelID: travelID, departaure: SpotFactory.fromUpperString(departure) ?? SeoulSpot(),
                      arrival: SpotFactory.fromUpperString(arrive) ?? SeoulSpot(), startDate: startDate.toDate()!, endDate: endDate.toDate()!, participant: participants, keywords: mappedKeywords, color: TicketColor(rawValue: color) ?? .blue)
    }
}
