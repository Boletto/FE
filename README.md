...
### Metal 기반 고급 이미지 필터 시스템

Metal Shading Language(MSL)를 활용한 커스텀 이미지 필터 구현
GPU 가속을 통한 실시간 이미지 프로세싱으로 부드러운 사용자 경험 제공
다양한 스타일의 필터 제공:

셀카: 자연스러운 색감과 선명도를 높여주는 기본 필터

음식: 음식 사진의 컬러와 질감을 강화해주는 전용 필터

네온: 밝은 색상을 강조하는 네온 효과 필터

레트로: 클래식한 필름 느낌의 복고풍 스타일

공포영화: 어두운 분위기와 비네팅으로 공포영화 같은 분위기 연출

수채화: 부드러운 색상 전환과 질감으로 수채화 효과 제공

손그림: 엣지 검출 알고리즘을 활용한 손으로 그린듯한 스케치 효과
